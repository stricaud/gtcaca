...
# so nothing else is needed. After installing, just run `cacamacs` in a terminal.
#
...
cp "$BUILD/apps/cacamacs"                 "$DEST/bin/"
...
LIBCACA="$(otool -L "$BUILD/apps/cacamacs" | awk '/libcaca/{print $1; exit}')"
...
install_name_tool -change "$LIBCACA" @rpath/libcaca.0.dylib "$DEST/bin/cacamacs"
# make sure cacamacs can find ../lib via rpath (it already does once installed;
# add it defensively in case the binary came straight from the build tree)
install_name_tool -add_rpath @executable_path/../lib "$DEST/bin/cacamacs" 2>/dev/null || true

echo ">> verifying the staged binary resolves only @rpath + system libs"
otool -L "$DEST/bin/cacamacs" | sed 's/^/   /'
...
rm -rf "$STAGE"
echo ">> done: $OUT"
echo "   install it (double-click or: sudo installer -pkg \"$OUT\" -target /), then run: cacamacs"
